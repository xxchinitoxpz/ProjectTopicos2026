    ...
    <turbo-frame id="modal" data-modal-size="max-w-2xl">
        <div data-modal-size="max-w-2xl" 
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Registrar Asistencia</h3>
                    <p class="text-xs text-gray-400">Registra una nueva asistencia para el personal.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.assistance.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="date" name="date" type="date" value="{{ old('date', date('Y-m-d')) }}" class="block mt-1 w-full text-sm" required />
                ...
                <div>
                    ...
                    <select id="state" name="state" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm" required>
                        <option value="presente" {{ old('state', 'presente') == 'presente' ? 'selected' : '' }}>Presente</option>
                        <option value="ausente" {{ old('state') == 'ausente' ? 'selected' : '' }}>Ausente</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" placeholder="Ej. Retraso de tráfico, permiso excepcional, etc..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm">{{ old('notes') }}</textarea>
