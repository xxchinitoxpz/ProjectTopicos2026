    ...
    <turbo-frame id="modal" data-modal-size="max-w-2xl">
        <div class="max-w-2xl mx-auto space-y-6 p-6 sm:p-8">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Registrar Marca</h3>
                    <p class="text-xs text-gray-400">Agrega un fabricante a la base de datos.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.brand.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf

                <!-- Name Field -->
                <div>
                    <x-input-label for="name" :value="__('Nombre de la Marca')" />
                    <x-text-input id="name" name="name" type="text" value="{{ old('name') }}" placeholder="Ej. Toyota, Volvo, etc." class="block mt-1 w-full text-sm" required autofocus />
                ...
                <div>
                    ...
                    <div class="flex items-center gap-6 mt-1.5">
                        <div class="w-16 h-16 rounded-xl border border-gray-200 shadow-sm overflow-hidden bg-gray-50 flex items-center justify-center">
                            <img id="logo_preview" src="https://ui-avatars.com/api/?name=M&color=10B981&background=f0fdf4&size=128" class="w-full h-full object-cover hidden" alt="Vista previa">
                            <span id="logo_placeholder" class="text-gray-400 text-xs font-semibold">Sin logo</span>
                        </div>
                        <div class="flex-1">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Detalle adicional sobre el fabricante..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm">{{ old('description') }}</textarea>
