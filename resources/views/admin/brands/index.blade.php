    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <form action="{{ route('admin.brand.index') }}" method="GET" class="flex flex-1 gap-2">
                    ...
                    @if($search)
                        <a href="{{ route('admin.brand.index', ['per_page' => $perPage]) }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-bold rounded-xl transition flex items-center">
                            Limpiar
                        </a>
                    @endif
                </form>
                <form action="{{ route('admin.brand.index') }}" method="GET" class="flex items-center gap-2 text-sm text-gray-500">
