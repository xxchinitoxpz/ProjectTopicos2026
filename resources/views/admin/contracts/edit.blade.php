    ...
    <turbo-frame id="modal" data-modal-size="max-w-4xl">
        <div data-modal-size="max-w-4xl" class="max-w-4xl mx-auto space-y-6 p-6 sm:p-8">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Modificar Contrato</h3>
                    <p class="text-xs text-gray-400">Edita los términos y condiciones del contrato de <strong>{{ $contract->staff?->name }} {{ $contract->staff?->last_name }}</strong>.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.contract.update', $contract->id) }}" method="POST" x-data="{ contractType: '{{ old('contract_type', $contract->contract_type) }}' }" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="staff_id" name="staff_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm bg-gray-50" required>
                            @foreach($staffMembers as $staff)
                                <option value="{{ $staff->id }}" {{ old('staff_id', $contract->staff_id) == $staff->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="date_start" name="date_start" type="date" value="{{ old('date_start', $contract->date_start?->format('Y-m-d')) }}" class="block mt-1 w-full text-sm" required />
                    ...
                    <div x-show="contractType === 'temporal'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0">
                        ...
                        <x-text-input id="date_end" name="date_end" type="date" value="{{ old('date_end', $contract->date_end?->format('Y-m-d')) }}" ::disabled="contractType !== 'temporal'" class="block mt-1 w-full text-sm" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <x-text-input id="salary" name="salary" type="number" step="0.01" min="0" value="{{ old('salary', $contract->salary) }}" placeholder="Ej. 1500.00" class="block mt-1 w-full text-sm" required />
                        <x-input-error :messages="$errors->get('salary')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="probation" :value="__('Período de Prueba (Meses)')" />
                        <x-text-input id="probation" name="probation" type="number" step="1" min="0" value="{{ old('probation', $contract->probation) }}" placeholder="Ej. 3 (Opcional)" class="block mt-1 w-full text-sm" />
                    ...
                    <div>
                        ...
                        <select id="state" name="state" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm" required>
                            <option value="active" {{ old('state', $contract->state) == 'active' ? 'selected' : '' }}>Activo</option>
                            <option value="inactive" {{ old('state', $contract->state) == 'inactive' ? 'selected' : '' }}>Inactivo</option>
                            <option value="expired" {{ old('state', $contract->state) == 'expired' ? 'selected' : '' }}>Vencido</option>
                        </select>
