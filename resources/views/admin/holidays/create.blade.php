    ...
    <turbo-frame id="modal" data-modal-size="max-w-2xl">
        <div data-modal-size="max-w-2xl" class="max-w-2xl mx-auto space-y-6 p-6 sm:p-8">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Registrar Feriado</h3>
                    <p class="text-xs text-gray-400">Agrega un nuevo día no laborable al calendario.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.holiday.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="date" name="date" type="date" value="{{ old('date', date('Y-m-d')) }}" class="block mt-1 w-full text-sm" required autofocus />
                    ...
                    <div>
                        ...
                        <select id="state" name="state" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm" required>
                            <option value="activo" {{ old('state', 'activo') === 'activo' ? 'selected' : '' }}>Activo</option>
                ...
                <div>
                    ...
                    <x-text-input id="description" name="description" type="text" value="{{ old('description') }}" placeholder="Ej. Año Nuevo, Día del Trabajador, etc." class="block mt-1 w-full text-sm" required />
