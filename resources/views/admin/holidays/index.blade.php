<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div>
                <h3 class="text-lg font-bold text-usat-blue">Listado de Feriados</h3>
                <p class="text-xs text-gray-400">Gestiona los días feriados o no laborables registrados en la plataforma.</p>
            </div>
            <div>
                <a href="{{ route('admin.holiday.create') }}" data-turbo-frame="modal" class="inline-flex items-center px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-sm font-semibold rounded-xl transition shadow-lg shadow-emerald-600/10">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <form action="{{ route('admin.holiday.index') }}" method="GET" class="flex flex-1 gap-2">
                    <input type="hidden" name="per_page" value="{{ $perPage }}">
                    <x-text-input type="text" name="search" value="{{ $search }}" placeholder="Buscar por descripción..." class="flex-1 text-sm" />
                    ...
                    @if($search || $stateFilter)
                        <a href="{{ route('admin.holiday.index', ['per_page' => $perPage]) }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-bold rounded-xl transition flex items-center">
                            Limpiar
                        </a>
                    @endif
                </form>
                <form action="{{ route('admin.holiday.index') }}" method="GET" class="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-700">
                        @forelse($holidays as $holiday)
                            <tr class="hover:bg-gray-50/40 transition">
                                ...
                                <td class="py-3.5 px-6 text-gray-650 font-semibold capitalize">
                                    {{ $holiday->day_name }}
                                </td>
                                ...
                                <td class="py-3.5 px-6 text-center">
                                    ...
                                    @else
                                        <span class="px-2.5 py-1 text-[10px] font-bold bg-red-50 text-red-650 rounded-full border border-red-100">Inactivo</span>
                                    @endif
                                </td>
                                <td class="py-3.5 px-6">
                                    <div class="flex items-center justify-center space-x-2">
                                        <a href="{{ route('admin.holiday.edit', $holiday->id) }}" data-turbo-frame="modal" class="p-2 bg-amber-50 text-usat-gold hover:bg-amber-100 rounded-lg transition duration-150" title="Editar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.holiday.destroy', $holiday->id) }}" method="POST" onsubmit="return confirm('¿Está seguro de eliminar este feriado?');" class="inline">
                                            @csrf
                                            @method('DELETE')
