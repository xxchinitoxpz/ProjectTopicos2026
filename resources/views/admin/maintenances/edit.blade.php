<x-app-layout>
    ...
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div>
                <h3 class="text-lg font-bold text-usat-blue">Modificar Mantenimiento</h3>
                <p class="text-xs text-gray-400">Actualiza el periodo de mantenimiento seleccionado.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.maintenance.update', $maintenance->id) }}" method="POST" class="p-6 space-y-6" data-turbo="false" data-ajax-form>
                ...
                <div>
                    ...
                    <x-text-input id="nombre" name="nombre" type="text" value="{{ old('nombre', $maintenance->nombre) }}" class="block mt-1 w-full text-sm" required autofocus />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="fecha_inicio" name="fecha_inicio" type="date" value="{{ old('fecha_inicio', $maintenance->fecha_inicio->format('Y-m-d')) }}" class="block mt-1 w-full text-sm" required onchange="document.getElementById('fecha_fin').min = this.value; if (document.getElementById('fecha_fin').value < this.value) document.getElementById('fecha_fin').value = this.value;" />
                        <x-input-error :messages="$errors->get('fecha_inicio')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="fecha_fin" :value="__('Fecha de Fin')" />
                        <x-text-input id="fecha_fin" name="fecha_fin" type="date" value="{{ old('fecha_fin', $maintenance->fecha_fin->format('Y-m-d')) }}" min="{{ old('fecha_inicio', $maintenance->fecha_inicio->format('Y-m-d')) }}" class="block mt-1 w-full text-sm" required />
