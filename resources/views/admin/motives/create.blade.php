        ...
        <div data-modal-size="max-w-lg" class="w-full max-w-lg mx-auto overflow-hidden rounded-sm bg-white shadow-2xl">
            <div class="flex items-center justify-between bg-blue-900 px-4 py-3 text-white">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold">Nuevo Motivo</h3>
                </div>
            ...
            <form action="{{ route('admin.motive.store') }}" method="POST" class="p-5" data-turbo="false" data-turbo-frame="_top">
                @csrf

                <div class="space-y-4">
                    <div>
                        <x-input-label for="name" :value="__('Nombre')" />
                        <x-text-input id="name" name="name" type="text" value="{{ old('name') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-200 focus:ring text-sm" placeholder="Nombre del motivo" required autofocus />
                        <x-input-error :messages="$errors->get('name')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="description" :value="__('Descripción')" />
                        <textarea id="description" name="description" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-200 focus:ring text-sm" placeholder="Agregue una descripción">{{ old('description') }}</textarea>
