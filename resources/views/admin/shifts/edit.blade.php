    ...
    <turbo-frame id="modal" data-modal-size="max-w-2xl">
        <div data-modal-size="max-w-2xl" class="max-w-2xl mx-auto space-y-6 p-6 sm:p-8">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Modificar Turno</h3>
                    <p class="text-xs text-gray-400">Edita los detalles del horario: {{ $shift->name }}.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.shift.update', $shift->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" value="{{ old('name', $shift->name) }}" placeholder="Ej. Turno Mañana, Turno Noche, etc." class="block mt-1 w-full text-sm" required autofocus />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="check_in" name="check_in" type="time" value="{{ old('check_in', substr($shift->check_in, 0, 5)) }}" class="block mt-1 w-full text-sm" required />
                        <x-input-error :messages="$errors->get('check_in')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="check_out" :value="__('Hora de Salida')" />
                        <x-text-input id="check_out" name="check_out" type="time" value="{{ old('check_out', substr($shift->check_out, 0, 5)) }}" class="block mt-1 w-full text-sm" required />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Ej. Incluye refrigerio de 13:00 a 14:00, lunes a viernes..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm">{{ old('description', $shift->description) }}</textarea>
