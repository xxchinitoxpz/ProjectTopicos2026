<x-app-layout>
    <x-slot:title>Editar Grupo de Personal</x-slot:title>

    <div class="space-y-4">
        <div class="flex items-center justify-between bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            <div>
                <h3 class="text-lg font-bold text-usat-blue">Editar Grupo: {{ $staffGroup->name }}</h3>
                <p class="text-xs text-gray-400">Modifica los detalles del equipo de trabajo y su programación.</p>
            </div>
        ...
        <form action="{{ route('admin.staff-group.update', $staffGroup->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" value="{{ old('name', $staffGroup->name) }}"
                    ...
                    <div>
                        ...
                        <select id="shift_id" name="shift_id"
                            ...
                            @foreach($shifts as $shift)
                                <option value="{{ $shift->id }}" {{ old('shift_id', $staffGroup->shift_id) == $shift->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="zone_id" name="zone_id"
                            ...
                            @foreach($zones as $zone)
                                <option value="{{ $zone->id }}" {{ old('zone_id', $staffGroup->zone_id) == $zone->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="vehicle_id" name="vehicle_id"
                            ...
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $staffGroup->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 text-sm" required>
                            <option value="active" {{ old('status', $staffGroup->status) === 'active' ? 'selected' : '' }}>Activo</option>
                            <option value="inactive" {{ old('status', $staffGroup->status) === 'inactive' ? 'selected' : '' }}>Inactivo</option>
                        </select>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-2">
                            @foreach(['lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo'] as $day)
                                @php
                                    $savedDays = is_array($staffGroup->days) ? $staffGroup->days : [];
                                @endphp
                                <label class="flex items-center gap-2 p-2.5 border border-gray-100 hover:border-emerald-300 hover:bg-emerald-50/20 rounded-xl transition cursor-pointer text-xs font-semibold capitalize text-gray-700">
                                    <input type="checkbox" name="days[]" value="{{ $day }}" 
                                        {{ in_array($day, old('days', $savedDays)) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <select id="driver_id" name="driver_id"
                            ...
                            @foreach($drivers as $driver)
                                <option value="{{ $driver->id }}" {{ old('driver_id', $staffGroup->driver_id) == $driver->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="space-y-1.5 max-h-60 overflow-y-auto p-3 border border-gray-150 rounded-xl bg-gray-50/50">
                            @forelse($helpers as $helper)
                                @php
                                    $savedHelpers = $staffGroup->helpers->pluck('id')->toArray();
                                @endphp
                                <label class="flex items-center justify-between p-2 hover:bg-white rounded-lg transition border border-transparent hover:border-gray-100 cursor-pointer">
                                    <div class="flex items-center gap-2.5">
                                        <input type="checkbox" name="helpers[]" value="{{ $helper->id }}"
                                            {{ in_array($helper->id, old('helpers', $savedHelpers)) ? 'checked' : '' }}
                                            class="rounded text-emerald-600 focus:ring-emerald-500 border-gray-300">
                                        <div>
