    ...
    <turbo-frame id="modal" data-modal-size="max-w-2xl">
        <div class="max-w-2xl mx-auto space-y-6 p-6 sm:p-8">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Modificar Tipo de Personal</h3>
                    <p class="text-xs text-gray-400">Edita los detalles del tipo de personal: {{ $type->name }}.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.staff-type.update', $type->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" value="{{ old('name', $type->name) }}" placeholder="Ej. Chofer, Supervisor, etc." class="block mt-1 w-full text-sm" required autofocus />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Detalle adicional sobre las funciones de este tipo de personal..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm">{{ old('description', $type->description) }}</textarea>
