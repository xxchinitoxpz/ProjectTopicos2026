    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                <form action="{{ route('admin.staff.index') }}" method="GET" class="flex max-w-md gap-2">
                    <x-text-input type="text" name="search" value="{{ $search }}" placeholder="Buscar por DNI, nombre, email o cargo..." class="w-full text-sm" />
                    <button type="submit" class="px-4 py-2 bg-usat-blue hover:bg-blue-800 text-white text-sm font-bold rounded-xl transition">
                        Buscar
                    </button>
                    @if($search)
                        <a href="{{ route('admin.staff.index') }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-bold rounded-xl transition flex items-center">
                            Limpiar
                        </a>
                    @endif
                </form>
