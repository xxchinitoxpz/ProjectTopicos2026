    ...
    <turbo-frame id="modal" data-modal-size="max-w-2xl">
        <div class="max-w-2xl mx-auto space-y-6 p-6 sm:p-8">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Registrar Color</h3>
                    <p class="text-xs text-gray-400">Agrega un nuevo tono a la base de datos.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.vehicle-color.store') }}" method="POST" class="space-y-6">
                @csrf

                <!-- Name Field -->
                <div>
                    <x-input-label for="name" :value="__('Nombre del Color')" />
                    <x-text-input id="name" name="name" type="text" value="{{ old('name') }}" placeholder="Ej. Verde Ecológico" class="block mt-1 w-full text-sm" required autofocus />
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4 mt-1.5">
                        <input type="color" id="color_picker" value="{{ old('color_picker', '#10b981') }}" class="w-16 h-11 rounded-lg border border-gray-200 cursor-pointer p-0 overflow-hidden bg-transparent" />
                        <x-text-input id="code" name="code" type="text" value="{{ old('code', '#10B981') }}" placeholder="#10B981" class="w-36 text-sm font-semibold tracking-wider text-center" required readonly />
                    </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Detalle adicional sobre el uso del color..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm">{{ old('description') }}</textarea>
