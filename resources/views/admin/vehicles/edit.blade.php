    ...
    <turbo-frame id="modal" data-modal-size="max-w-4xl">
        <div data-modal-size="max-w-4xl" class="max-w-4xl mx-auto space-y-6 p-6 sm:p-8">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-usat-blue">Modificar Vehículo</h3>
                    <p class="text-xs text-gray-400">Edita los detalles y especificaciones técnicas: {{ $vehicle->name }}.</p>
                </div>
            ...
            <!-- Form Card -->
            <form action="{{ route('admin.vehicle.update', $vehicle->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" value="{{ old('name', $vehicle->name) }}" placeholder="Ej. Camión Recolector 1" class="block mt-1 w-full text-sm" required autofocus />
                        <x-input-error :messages="$errors->get('name')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="plate" :value="__('Placa / Matrícula')" />
                        <x-text-input id="plate" name="plate" type="text" value="{{ old('plate', $vehicle->plate) }}" placeholder="Ej. ABC-123" class="block mt-1 w-full text-sm" required />
                        <x-input-error :messages="$errors->get('plate')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="code" :value="__('Código Interno')" />
                        <x-text-input id="code" name="code" type="text" value="{{ old('code', $vehicle->code) }}" placeholder="Ej. V-01" class="block mt-1 w-full text-sm" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        <select id="model_id" name="model_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm" required>
                    ...
                    <div>
                        ...
                        <select id="color_id" name="color_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm" required>
                            ...
                            @foreach($colors as $color)
                                <option value="{{ $color->id }}" {{ old('color_id', $vehicle->color_id) == $color->id ? 'selected' : '' }}>{{ $color->name }} ({{ $color->code }})</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <x-text-input id="year" name="year" type="number" min="1900" max="{{ date('Y')+1 }}" value="{{ old('year', $vehicle->year) }}" placeholder="Ej. 2020" class="block mt-1 w-full text-sm" />
                        <x-input-error :messages="$errors->get('year')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="occupant_capacity" :value="__('Capacidad Ocupantes')" />
                        <x-text-input id="occupant_capacity" name="occupant_capacity" type="number" min="1" value="{{ old('occupant_capacity', $vehicle->occupant_capacity) }}" placeholder="Ej. 3" class="block mt-1 w-full text-sm" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <x-text-input id="load_capacity" name="load_capacity" type="number" step="0.01" min="0" value="{{ old('load_capacity', $vehicle->load_capacity) }}" placeholder="Ej. 3.50" class="block mt-1 w-full text-sm" />
                    ...
                    <div>
                        ...
                        <x-text-input id="compaction_capacity" name="compaction_capacity" type="number" step="0.01" min="0" value="{{ old('compaction_capacity', $vehicle->compaction_capacity) }}" placeholder="Ej. 5.00" class="block mt-1 w-full text-sm" />
                    ...
                    <div>
                        ...
                        <x-text-input id="combustible_capacity" name="combustible_capacity" type="number" step="0.01" min="0" value="{{ old('combustible_capacity', $vehicle->combustible_capacity) }}" placeholder="Ej. 15.00" class="block mt-1 w-full text-sm" />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Detalles de equipamiento, estado mecánico..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 text-sm">{{ old('description', $vehicle->description) }}</textarea>
