<x-app-layout>
    <x-slot:title>Editar Zona</x-slot:title>
    ...
    <div x-data="zoneEditForm()" class="space-y-4">

        <div class="flex items-center justify-between bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            <div>
                <h3 class="text-lg font-bold text-usat-blue">Editar Zona: {{ $zone->name }}</h3>
                <p class="text-xs text-gray-400">Arrastra los vértices para ajustar · Clic en el borde para añadir · Clic derecho en vértice para quitar</p>
            </div>
        ...
        <form action="{{ route('admin.zone.update', $zone->id) }}" method="POST" @submit.prevent="submitForm($event)">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" value="{{ old('name', $zone->name) }}"
                            class="block mt-1 w-full text-sm" required autofocus/>
                    ...
                    <div>
                        ...
                        <select x-model="deptId" @change="loadProvinces()"
                            ...
                            @foreach($departments as $d)
                                <option value="{{ $d->id }}" {{ $d->id == $zone->district->province->department_id ? 'selected' : '' }}>{{ $d->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select x-model="provId" @change="loadDistricts()" :disabled="!provinces.length"
                            ...
                            <template x-for="p in provinces" :key="p.id">
                                <option :value="p.id" :selected="p.id == initProvId" x-text="p.name"></option>
                            </template>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="district_id" :disabled="!districts.length"
                            ...
                            <template x-for="d in districts" :key="d.id">
                                <option :value="d.id" :selected="d.id == initDistrictId" x-text="d.name"></option>
                            </template>
                        </select>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 text-sm">{{ old('description', $zone->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <x-text-input id="avg_waste_kg" name="avg_waste_kg" type="number" step="0.01" min="0"
                            value="{{ old('avg_waste_kg', $zone->avg_waste_kg) }}" placeholder="Ej: 150.50" class="block mt-1 w-full text-sm"/>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 text-sm" required>
                            <option value="active"   {{ old('status', $zone->status) === 'active'   ? 'selected' : '' }}>Activo</option>
                            <option value="inactive" {{ old('status', $zone->status) === 'inactive' ? 'selected' : '' }}>Inactivo</option>
                        </select>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                    ...
                    <div class="p-3 border-b border-gray-100 flex items-center gap-2 flex-wrap">
                        ...
                        <div class="flex gap-1.5 flex-shrink-0">
                            {{-- Modo dibujo (si no hay polígono o se quiere redibujar) --}}
    ...
    function zoneEditForm() {
        const initCoords = @json($zone->zoneCoords->map(fn($c) => ['lat' => (string)$c->latitude, 'lng' => (string)$c->longitude]));
        const initArea   = {{ $zone->area ? (float)$zone->area : 0 }};

        return {
            deptId:         '{{ $zone->district->province->department_id }}',
            provId:         '{{ $zone->district->province_id }}',
            initProvId:     {{ $zone->district->province_id }},
            initDistrictId: {{ $zone->district_id }},
            provinces:  @json($provinces),
            districts:  @json($districts),
            coords:     JSON.parse(JSON.stringify(initCoords)),
            drawnLayer: null,
            isDrawing:  false,
            areaHa:     initArea ? (initArea * 100).toFixed(2) : '0.00',
            areaKm2:    initArea ? initArea.toFixed(6) : '',
            addrResults: [],
            map: null,
            ...
            init() {
                this.$nextTick(() => {
                    ...
                    // Cargar polígono existente y activar edición
                    if (initCoords.length >= 3) {
                        const pts = initCoords.map(c => [parseFloat(c.lat), parseFloat(c.lng)]);
                        this.drawnLayer = L.polygon([pts], {
                            color:'#10b981', fillColor:'#10b981', fillOpacity:0.2, weight:2.5
                        }).addTo(this.map);

                        // Activar modo edición inmediatamente
                        this.drawnLayer.pm.enable({ allowSelfIntersection: false });
                        this.drawnLayer.on('pm:change', () => this._syncCoords());

                        this.map.fitBounds(this.drawnLayer.getBounds(), { padding: [30, 30] });
                    }
                });
            },

            initMap() {
                const center = initCoords.length > 0
                    ? [parseFloat(initCoords[0].lat), parseFloat(initCoords[0].lng)]
                    : [-6.7714, -79.8409];

                this.map = L.map('draw-map').setView(center, 14);
            ...
            cancelDrawing() {
                ...
                this.isDrawing = false;
                // Restaurar polígono original si existe
                if (!this.drawnLayer && initCoords.length >= 3) {
                    this.coords = JSON.parse(JSON.stringify(initCoords));
                    const pts = initCoords.map(c => [parseFloat(c.lat), parseFloat(c.lng)]);
                    this.drawnLayer = L.polygon([pts], {
                        color:'#10b981', fillColor:'#10b981', fillOpacity:0.2, weight:2.5
                    }).addTo(this.map);
            ...
            clearAll() {
                if (!confirm('¿Eliminar el polígono? Se perderán las coordenadas actuales.')) return;
                if (this.drawnLayer) {
                    this.drawnLayer.pm.disable();
                    this.map.removeLayer(this.drawnLayer);
                    this.drawnLayer = null;
